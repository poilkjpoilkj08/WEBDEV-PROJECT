                                ...
                                <tbody>
                                    ...
                                    @foreach ($cart as $id => $item)
                                        @php 
                                            ...
                                            $maxStock = $product ? $product->stock : 10;
                                        @endphp
                                        <tr>
                                            <td class="ps-4 py-3">
                                            ...
                                            <td class="text-center py-3" style="width: 140px;">
                                                <form action="{{ route('update_cart', $id) }}" method="POST">
                                                    ...
                                                    <div class="input-group input-group-sm mx-auto shadow-sm rounded border overflow-hidden" style="max-width: 110px;">
                                                        ...
                                                        <input type="number" name="quantity" class="form-control text-center border-0 bg-white fw-bold px-1" value="{{ $item['quantity'] }}" min="1" max="{{ $maxStock }}" readonly>
                                                        <button class="btn btn-light border-0 border-start fw-bold px-2 text-muted" type="button" onclick="const input = this.previousElementSibling; const maxQty = {{ $maxStock }}; if(input.value < maxQty) { input.value++; this.form.submit(); } else { showMaxQuantityAlert({{ $maxStock }}); }">＋</button>
                                                    </div>
                                                </form>
                                            </td>
...
function showMaxQuantityAlert(maxQty) {
    const alertHTML = `
        <div class="alert alert-warning alert-dismissible fade show position-fixed top-0 start-50 translate-middle-x mt-4 shadow" style="z-index: 1050; width: auto; max-width: 90%; padding-right: 2.5rem;" role="alert" id="maxQtyAlert">
            <strong>Maximum Quantity!</strong> You can only add up to <strong>${maxQty}</strong> item(s) of this product.
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close" style="cursor: pointer;"></button>
        </div>
    `;
    
    // Remove any existing alert
    const existingAlert = document.getElementById('maxQtyAlert');
    if (existingAlert) {
    ...
    setTimeout(function() {
        const alert = document.getElementById('maxQtyAlert');
        if (alert) {
